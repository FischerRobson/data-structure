...
class TreeFormatter {
    static int padding = 4; // minimum number of horizontal spaces between two node data
    ...
    private static List<String> buildLines(Tree.Node node) {
        if (node == null) return new ArrayList<>();
        List<String> lines = merge(buildLines(node.left), buildLines(node.right));
        int half = String.valueOf(node.value).length() / 2;
        int i = half;
        if (!lines.isEmpty()) {
            String line;
        ...
        lines.add(0, " ".repeat(indent(lines, i - half)) + node.value);
        lines.add(0, " ".repeat(i + Math.max(0, half - i)) + "*"); // Add a marker for caller
        return lines;
    }

    public static String topDown(Tree.Node root) {
